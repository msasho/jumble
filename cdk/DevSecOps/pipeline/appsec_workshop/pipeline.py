...
class Pipeline(Construct):
    ...
    def __init__(self, scope: Construct, id: str, devtools, tasks, config: dict, **kwargs):
        ...
        if config['sast']['enabled']:
            ...
            sast.role.add_managed_policy(iam.ManagedPolicy.from_aws_managed_policy_name(
                ...
                environment_variables={
                    "ZAP_API_URL": codebuild.BuildEnvironmentVariable(
                        value="http://{}:8080".format(
                            devtools.zaproxy.instance_public_dns_name)
                    ),
                    ...
                    "SCAN_URL": codebuild.BuildEnvironmentVariable(
                        value="http://{}".format(
                            tasks.flask_app.load_balancer.load_balancer_dns_name)
                    )
                },
                description="DAST",
            )
